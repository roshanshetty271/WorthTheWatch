...
function loadSessionFilters() {
    ...
    try {
        const saved = sessionStorage.getItem("picks_filters");
...
export default function PicksPage() {
    ...
    const [worthIt, setWorthIt] = useState<PickResult[]>([]);
    ...
    const fetchResults = useCallback(async (pageNum: number = 1) => {
        ...
        try {
            ...
            if (genres.length) params.set("genres", genres.map((g) => g.toLowerCase()).join(","));

            const res = await fetch(`${API_BASE}/api/picks?${params.toString()}`, { signal: controller.signal });
            if (res.ok) {
    ...
    useEffect(() => {
        ...
        router.replace(`/picks?${params.toString()}`, { scroll: false });
        try {
            sessionStorage.setItem("picks_filters", JSON.stringify({ mediaType, genres, match, sort }));
        } catch {}
    }, [mediaType, genres, match, sort, router]);

    const toggleGenre = (g: string) =>
        setGenres((prev) => (prev.includes(g) ? prev.filter((x) => x !== g) : [...prev, g]));

    const clearFilters = () => { setGenres([]); setMatch("any"); setSort("popular"); };

    const genreLabel = genres.length ? genres.join(" + ") : "Right Now";
    const hasFilters = genres.length > 0 || sort !== "popular" || match !== "any";
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-white">
            ...
            <div className="pt-28 md:pt-32 pb-2 px-4">
                <div className="max-w-6xl mx-auto">
                    ...
                    <p className="text-white/40 mt-2 text-sm max-w-md">
                        Tell us the vibe. We&apos;ll surface what&apos;s actually worth it — then more to explore.
                    </p>
                </div>
            </div>
            ...
            <div className="sticky top-16 z-30 bg-[#0a0a0a]/95 backdrop-blur-md border-b border-white/5 py-4 px-4">
                <div className="max-w-6xl mx-auto space-y-3">
                    {/* Row 1: Media type · Match toggle · Sort · Clear */}
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {/* Match any/all — only meaningful with 2+ genres */}
                        {genres.length >= 2 && (
            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                {loading && worthIt.length === 0 && results.length === 0 ? (
                    ...
                    <div className="text-center py-16 animate-fade-in">
                        ...
                        <h3 className="font-display text-xl text-white/60 mb-2">Couldn&apos;t load picks</h3>
                    ...
                    <>
                                    ...
                                    {worthIt.map((item) => (
                                        ...
                                        >
                                            ...
                                            {item.hook ? (
                                                ...
                                                <p className="text-[10px] text-white/30">{item.release_date.split("-")[0]}</p>
                                            ) : null}
                                        </Link>
                                    ))}
                                </div>
                        ...
                        {worthIt.length === 0 && results.length === 0 && !loading && (
                            <div className="text-center py-16">
                                ...
                                <h3 className="text-xl font-bold text-white/60 mb-2">No picks found</h3>
                                <p className="text-sm text-white/30">Try fewer genres or switch to &ldquo;Any of these&rdquo;.</p>
